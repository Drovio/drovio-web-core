 ...
 * @revised	October 1, 2014, 12:39 (EEST)
 */
class WSPage extends HTMLPagePrototype
{
	...
	private function addResources()
	{
		...
		// TODO : Uncomment when Url class is completed
	...
	private function populateBody($pagePath = "")
	{
		...
		$pageViewContainer = HTML::create("div", "", "viewContainer", "uiViewContainer");
		HTML::append($mainContainer, $pageViewContainer);
		
		// Load page view
		$viewContent = $this->loadView($pagePath);
		
		// Append Website page view content		
		//$holderElement = HTML::select("#viewContainer")->item(0);
	...
	private function loadView($pagePath)
	{		
		$websitePagesPath = '';
		
		// Normalize page path
		$pagePath = (empty($pagePath) ? "index.php" : $pagePath);
		
		// Append website pages root folder path
		$pagePath = $websitePagesPath.$path;
		try
		{
			// Load the page view
