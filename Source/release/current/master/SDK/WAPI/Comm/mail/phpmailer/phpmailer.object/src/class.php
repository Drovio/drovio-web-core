 ...
 * 
 * @version	0.1-1
 * @created	July 8, 2014, 9:53 (EEST)
 * @revised	November 10, 2014, 12:51 (EET)
 */
class PHPMailer
{
	/**
	...
	/**
	 * Sets the message Date to be used in the Date header. If empty, the current date will be added.
	 * 
	 * @type	string
	 */
	public $MessageDate       = '';
	...
	/**
	 * Used with DKIM Signing required parameter if DKIM is enabled domain selector example domainkey
	 * 
	 * @type	string
	 */
	...
	/**
	 * Used with DKIM Signing. Optional parameter if your private key requires a passphrase.
	 * 
	 * @type	string
	 */
	...
	/**
	 * Callback Action function name. The function that handles the result of the send email action. It is called out by Send() for each email sent.
	 * 
	 * @type	string
	 */
	...
	/**
	 * What to use in the X-Mailer header. NULL for default, whitespace for None, or actual string to use
	 * 
	 * @type	string
	 */
	public $XMailer         = '';
	...
	/**
	 * An array of reply-to name and address
	 * 
	 * @type	array
	 */
	...
	/**
	 * An array of MIME boundary strings
	 * 
	 * @type	array
	 */
	...
	/**
	 * The filename of a DKIM certificate file
	 * 
	 * @type	string
	 */
	...
	/**
	 * The filename of a DKIM key file
	 * 
	 * @type	string
	 */
	 ...
	 * 		If true, sets the message type to HTML.
	 * 
	 * @return	void
	 */
	 ...
	 * Conforms to RFC5322: Uses *correct* regex on which FILTER_VALIDATE_EMAIL is based; So why not use FILTER_VALIDATE_EMAIL? Because it was broken to not allow a@b type valid addresses :(
	 * 
	 * @param	string	$address
	...
	public function PreSend()
	{
		try
		{
			...
			if ((count($this->to) + count($this->cc) + count($this->bcc)) < 1)
				throw new Exception("You must provide at least one recipient email address.", self::STOP_CRITICAL);
			...
			if (empty($this->Body))
				throw new Exception("Message body empty", self::STOP_CRITICAL);
		...
		if ($this->SingleTo === true)
		{
			foreach ($this->SingleToArray as $val)
			{
				if (!@$mail = popen($sendmail, 'w'))
					throw new Exception("Could not execute: " . $this->Sendmail, self::STOP_CRITICAL);
	 ...
	 * 
	 * @return	boolean
	 * 		True on success, false if there is a bad MAIL FROM, RCPT, or DATA input.
	 * 
	 * @throws	Exception
	 */
	protected function SmtpSend($header, $body)
	{
		...
		$bad_rcpt = array();
		if (!$this->SmtpConnect())
			throw new Exception('SMTP Connect() failed.', self::STOP_CRITICAL);

		$smtp_from = ($this->Sender == '') ? $this->From : $this->Sender;
		if (!$this->smtp->Mail($smtp_from))
			throw new Exception('The following From address failed: ' . $smtp_from, self::STOP_CRITICAL);
		...
		if (count($bad_rcpt) > 0 )
		{
			...
			throw new Exception('SMTP Error: The following recipients failed: ' . $badaddresses);
		}
		
		if (!$this->smtp->Data($header . $body))
			throw new Exception('SMTP Error: Data not accepted.', self::STOP_CRITICAL);
	...
	public function UTF8CharBoundary($encodedText, $maxLength)
	{
		...
		while (!$foundSplitPos)
		{
			...
			else // No encoded character found
				$foundSplitPos = true;
		}
		return $maxLength;
	}
	...
	/**
	 * Gets the MIME message (headers and body).
	 ...
	 * 
	 * @return	string
	 * 		The assembled message body or an empty string on failure.
	 * 
	 * @throws	Exception
	 */
	...
	/**
	 * Gets the start of a message boundary.
	 * 
	...
	/**
	 * Gets the end of a message boundary.
	 * 
	 * @param	string	$boundary
	 ...
	 * 
	 * @return	boolean
	 * 		True on success, false if the file could not be found or accessed.
	 * 
	 * @throws	Exception
	 */
	public function AddAttachment($path, $name = '', $encoding = 'base64', $type = 'application/octet-stream')
	{
		...
		{
			if ( !@is_file($path) )
				throw new Exception('Could not access file: ' . $path, self::STOP_CONTINUE);
	...
	/**
	 * Gets the current array of attachments.
	 * 
	 * @return	array
	 * 		An array including all the attachments.
	 */
	 ...
	 * 
	 * @return	string
	 * 		The attachment string on success, an empty string otherwise.
	 */
	 ...
	 * 
	 * @return	string
	 * 		The encoded file or an empty string on failure.
	 */
	protected function EncodeFile($path, $encoding = 'base64')
	{
		...
		{
			if (!is_readable($path))
				throw new Exception('File Error: Could not open file: ' . $path, self::STOP_CONTINUE);
	 ...
	 * 
	 * @return	string
	 * 		The encoded string or an empty string on failure.
	 */
	 ...
	 * 		Where the header is going to be used, see the RFC for what that means.
	 * 
	 * @return	string
	 * 		The encoded header.
	 */
	 ...
	 * 
	 * @return	boolean
	 * 		True if has multi bytes, false otherwise.
	 */
	...
	/**
	 * Checks if an inline attachment is present
	 * 
	 * @return	boolean
	 * 		Returns true if an inline attachment is present, false otherwise.
	 */
	...
	/**
	 * Checks if an attachment (non-inline) is present
	 * 
	 * @return	boolean
	 * 		Returns true if an attachment (non-inline) is present, false otherwise.
	 */
	 ...
	 * 
	 * @return	boolean
	 * 		True if this message have an alternative body set, false otherwise.
	 */
	...
	protected function SetError($msg)
	{
		...
		if ($this->Mailer == 'smtp' and !is_null($this->smtp))
		{
			...
			if (!empty($lasterror) and array_key_exists('smtp_msg', $lasterror))
				$msg .= 'SMTP server error: ' . $lasterror['smtp_msg'] . "\n";
		}
		$this->ErrorInfo = $msg;
	}
	
	/**
	 * Gets the proper RFC 822 formatted date.
	 * 
	 * @return	string
	...
	/**
	 * Gets the MIME type according to the given extension string.
	 * 
	 * @param	string	$ext
	 * 		The file extension.
	 * 
	 * @return	string
	 * 		The mime type according to the given extension.
	 */
	...
	public function set($name, $value = '')
	{
		try
		{
			if (isset($this->$name) )
				...
				throw new Exception("Cannot set or reset variable:" . $name, self::STOP_CRITICAL);
		}
		catch (Exception $e)
		{
			$this->SetError($e->getMessage());
	...
	public function DKIM_Add($headers_line, $subject, $body)
	{
		...
		return "X-PHPMAILER-DKIM: Redback Phpmailer\r\n".$dkimhdrs.$signed."\r\n";
	}
