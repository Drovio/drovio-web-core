 ...
 * 
 * @version	0.6-8
 * @created	October 1, 2014, 10:39 (BST)
 * @updated	December 11, 2015, 13:49 (GMT)
 */
class WSPage extends HTMLPagePrototype
{
	...
	private function setHead($robots = "")
	{
		...
		// Website Icon
		$gbSettings = new globalSettings();
		$faviconPath = $gbSettings->get("favicon_url");
		$this->addIcon(url::resource(wsrsrcRoot."/".$faviconPath));
	...
	private function addOpenGraphMeta()
	{
		...
		if ($metaSettings->get("meta_og_enabled"))
		{
			...
			{
				// Resolve image url
				$imageUrl = url::resource(wsrsrcRoot."/".$content);
				
				// Add meta
				$this->addMeta($name = "og:image", $imageUrl, $httpEquiv = "", $charset = "");
			}
	...
	private function addResources($pagePath)
	{
		...
		$this->addScript("//cdn.drov.io/js/jquery/jquery-2.1.4.min.js");
		...
		{
			// Check if there is a css file and load
			$cssResource = "/css/t/".$this->getFileName("css", $templateName, $themeName, "css").".css";
			if (file_exists(wsystemRoot.wslibRoot.$cssResource))
				$this->addStyle(url::resource(wslibRoot.$cssResource));
		
			// Check if there is a javascript file and load
			$jsResource = "/js/t/".$this->getFileName("js", $templateName, $themeName, "js").".js";
