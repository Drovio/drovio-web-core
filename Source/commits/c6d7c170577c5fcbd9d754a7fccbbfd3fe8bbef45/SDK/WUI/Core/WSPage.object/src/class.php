 ...
 * @updated	January 4, 2015, 0:42 (EET)
 */
class WSPage extends HTMLPagePrototype
{
	...
	public function build($pagePath = "")
	{
		...
		// Include meta
		$metaDescription = getPageSettingsValue($pageSettings, $metaSettings, "meta_description");
		$metaKeywords = getPageSettingsValue($pageSettings, $metaSettings, "meta_keywords");
	...
	private function addOpenGraphMeta()
	{
		...
		{
			// Site name
			$content = getPageSettingsValue($pageSettings, $metaSettings, "meta_og_sitename");
			if (!empty($content))
				$this->addMeta($name = "og:site_name", $content, $httpEquiv = "", $charset = "");
			
			// Site type
			$content = getPageSettingsValue($pageSettings, $metaSettings, "meta_og_type");
			if (!empty($content))
				$this->addMeta($name = "og:type", $content, $httpEquiv = "", $charset = "");
			
			// Image
			$content = getPageSettingsValue($pageSettings, $metaSettings, "meta_og_image");
			if (!empty($content))
				$this->addMeta($name = "og:image", $content, $httpEquiv = "", $charset = "");
			
			// Page title
			$content = getPageSettingsValue($pageSettings, $metaSettings, "meta_og_title");
			if (!empty($content))
