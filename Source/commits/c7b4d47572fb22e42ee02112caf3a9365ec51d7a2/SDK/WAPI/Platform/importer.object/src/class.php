 ...
 * @revised	December 30, 2014, 10:25 (EET)
 */
class importer
{
	/**
	...
	/**
	 * The web core's exported latest root folder.
	 * 
	 * @type	string
	 */
	private static $latestPath = "/System/Library/Web/SDK/";
	...
	public static function import($library, $package, $class = "")
	{
		...
		if (self::onDevelopment() && self::getTesterStatus($library, $package)))
			$nspath = self::$vcsPath.$library."/".$package."/".$class."/".self::$innerClassPath;
		else if (self::onDevelopment())
			$nspath = self::$latestPath.$library."/".$package."/".$class.".php";
		else
	 ...
	 * @return	void
	 */
	private function loadPackage($library, $package)
	{
		...
		if (self::onDevelopment() && self::getTesterStatus($library, $package))
		{
			// Load from web core repository
			$sourceMap = new sourceMap(systemRoot.self::$vcsPath, "map.xml");
		...
		{
			// Load from latest web core
			$sourceMap = new sourceMap(systemRoot.self::$latestPath, "map.xml");
			$pkgObjects = $sourceMap->getObjectList($library, $package);
			foreach ($pkgObjects as $object)
			{
				$objectName = (empty($object['namespace']) ? "" : $object['namespace']."/").$object['name'];
				importer::incl(self::$latestPath."/".$library."/".$package."/".$objectName.".php", TRUE, TRUE);
			}
		}
		else
		{
			// Load from website core
			$sourceMap = new sourceMap(systemRoot.systemWSDK, "map.xml");
			$pkgObjects = $sourceMap->getObjectList($library, $package);
			foreach ($pkgObjects as $object)
			{
				$objectName = (empty($object['namespace']) ? "" : $object['namespace']."/").$object['name'];
	...
	{
		return (is_null(self::getCookie("wsdkTester")) ? FALSE : self::getCookie("wsdkTester"));
	}
